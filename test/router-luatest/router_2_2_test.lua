...
g.test_master_search_not_log_vhandshake = function(g)
    local new_cluster_cfg = get_auto_master_global_cfg()
    vtest.router_cfg(g.router, new_cluster_cfg)
    local rs_uuid = g.replica_2_a:replicaset_uuid()
    local r_uuid = g.replica_2_a:instance_uuid()
    g.router:exec(function(rs_uuid)
        local rs = ivshard.router.internal.static_router.replicasets[rs_uuid]
        rs.master.conn.vconnect = {}
    end, {rs_uuid})
    ...
    local msg = "Error during master search.*VHANDSHAKE"
    t.assert_not(g.router:grep_log(msg))

    g.router:exec(function(rs_uuid, r_uuid)
        local rs = ivshard.router.internal.static_router.replicasets[rs_uuid]
        rs.replicas[r_uuid].conn.vconnect = nil
    end, {rs_uuid, r_uuid})
    vtest.router_cfg(g.router, global_cfg)
end
